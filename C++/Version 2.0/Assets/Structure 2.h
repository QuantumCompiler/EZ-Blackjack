    ...
    if (playerHand.player.bank_total >= playerHand.player.hand_wager) {
        if (check_same_parameter_in_hand(playerHand, "R", "Ace")) {
            ...
            while (aces_response.empty()) {
                cout << "\n" "You have the same rank of " << playerHand.player.cards[0].display_rank << " in your hand."
                << " Would you like to split your hand? You may only split your hand once (y/n): ";
                cin >> aces_response;
                cout << endl;
                if (aces_response == "y") {
            ...
            while (same_rank_response.empty()) {
                cout << "\n" "You have the same rank of " << playerHand.player.cards[0].display_rank << " in your hand."
                << " Would you like to split your hand? You may split up to a total of four times. (y/n): ";
                        ...
                        if (playerHand.player.bank_total >= playerHand.player.hand_wager) {
                            ...
                            while (split_counter < 4 && check_same_parameter_in_hand(checking_hand, "R")) {
                                ...
                                while (split_again != "y" || split_again != "n") {
                                    time_sleep(1);
                                    cout << endl << "You have pulled the same rank of " << checking_hand.player.cards[0].display_rank
                                    << " again. Would you like to split your current hand again? (y/n): ";
                                    cin >> split_again;
                                    cout << endl;
                                    if (split_again == "y") {
                                        split_counter++;
...
// Hand logic method
/*
* This method processes the logic for if a player wants to hit, stay, or double down on a current hand
...
* Depending on certain conditions, a player will have the option to hit, stay, or double down on the current hand that
    ...
    while ((response != "h" && response != "s" && response != "d") && playerHand.player.cards_total < 21) {
        if (!has_hit) {
            if (split_hand) {
                ...
                dealerHand.Show_Hand("initial");
                time_sleep(1);
                cout << endl << "Would you like to hit (h), stay (s), or double down (d) for hand " << to_string(hand_counter) << "? (h/s/d): ";
                cin >> response;
                cout << endl;
            }
            else if (!split_hand) {
                cout << endl << "Would you like to hit (h), stay (s), or double down (d)? (h/s/d): ";
                cin >> response;
                cout << endl;
            }
            else {}
            ...
            dealerHand.Show_Hand("initial");
            time_sleep(1);
            cout << endl << "Would you like to hit (h) or stay (s)? (h/s): ";
            cin >> response;
            cout << endl;
        }
        else {}
        if (response == "h") {
            ...
            time_sleep(1);
            cout << playerHand.player.name << " has chosen to stay. Final hand total: " << color_text(36, to_string(playerHand.player.cards_total)) << "." << endl;
            time_sleep(1);
            if (split_hand) {
                cout << endl << "Here is " << playerHand.player.name << "'s final hand for hand " << to_string(hand_counter) 
                << " and the " << dealerHand.player.name << "'s current hand:" << endl; 
...
void play_game() {
    ...
    while (gameShoe.deck.cards.size() >= 13 && playerHand.player.bank_total > 0) {
        ...
        if (playerHand.player.bank_total > 0) {
            ...
            time_sleep(1);
            cout << endl << playerHand.player.name << " has no currency left in their bank. Game over." << endl;
            time_sleep(1);
