...
bool BloomFilter::Contains(const std::string& input) {
    ...
    // Return boolean value at the index of the container in the filter
    return this->GetFilter()[tempBucket];
}
...
void BloomFilter::RemoveElement(const std::string& input) {
    ...
    unsigned int tempHash = this->CalculateHashCode(input);
    unsigned int tempBucket = this->CalculateBucketIndex(tempHash);
    // Change the boolean value at the index of the container in the filter to false
